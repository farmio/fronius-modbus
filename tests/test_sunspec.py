...
from fronius_modbus.sunspec import discover_models
from fronius_modbus.testing import MpptModuleSpec, build_sunspec_map


async def test_discover_models(mock_modbus_unit: MockModbusUnit) -> None:
    ...
    models = await discover_models(mock_modbus_unit)
    assert models == [
...
async def test_discover_models_no_sunspec(mock_modbus_unit: MockModbusUnit) -> None:
    """Test model discovery on a device without SunSpec marker."""
    with pytest.raises(SunSpecError, match="No SunSpec marker"):
        await discover_models(mock_modbus_unit)


async def test_discover_models_unterminated(mock_modbus_unit: MockModbusUnit) -> None:
